...
from services.database.schemas.story import (
    StoryCreateSchema,
    StoryChangeSchema,
    StorySchema,
)
...
@router.get(
    ...
    description="Get favorite stories. Should be authorized as organization member",
)
async def get_favorite_stories_by_organization(
    ...
    if user.organization_id != organization_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="У вас нет прав на просмотр этого списка",
        )
